          ...
          {categories.map((category) => {
            return (
              <Link key={category.id} href={`/${locale}/products?category_id=${category.id}`} className="group flex min-h-40 flex-col justify-between rounded-[18px] bg-white p-5 shadow-[0_2px_18px_rgba(21,48,66,0.05)] transition hover:-translate-y-1 hover:shadow-[0_10px_30px_rgba(21,48,66,0.1)]">
                <CategoryIcon iconKey={category.icon_key} ancestorKeys={getCategoryAncestorIconKeys(category, categories)} label={`${category.name} category icon`} className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--sf-soft-blue)] text-xl text-[var(--sf-accent)]" />
                <div className="mt-6">
